...
class DiskCache(
  ...
  private val job = Job()
  private val cacheInfoFile = File(cacheDir, "disk-org.rakishou.cachingimageloader.cache.dat")
    ...
    runBlocking(job) {
      ...
      if (cacheInfoFile.exists()) {
        ...
        if (!cacheInfoFile.createNewFile()) {
          throw RuntimeException("Could not create org.rakishou.cachingimageloader.cache info file!")
        }
      }
    }
  }
  ...
  private fun innerReadCacheFile(cacheInfoRecordMap: HashMap<String, CacheInfoRecord>) {
    ...
    for (line in lines) {
      ...
      if (split.size != 4) {
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: split.size (${split.size}) != 5")
      ...
      if (url.isEmpty()) {
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: url is empty")
        hasCorruptedEntries = true
        continue
      }

      if (filePath.isEmpty()) {
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: filePath is empty")
        hasCorruptedEntries = true
        continue
      }
      ...
      if (!file.exists() || !file.isFile) {
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: file does not exist or it's not a file")
        hasCorruptedEntries = true
        continue
      }
      ...
      val addedOn = try {
        ...
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: addedOnStr is not a number")
        hasCorruptedEntries = true
        continue
      }
      ...
      if (cacheInfoRecordMap.containsKey(url)) {
        debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: cacheInfoRecordMap already contain the url ($url)")
        hasCorruptedEntries = true
        continue
      }
    ...
    if (fileCount != lines.size) {
      debugPrint("Corrupted org.rakishou.cachingimageloader.cache info file: count of files in the org.rakishou.cachingimageloader.cache directory ($fileCount) != entries in the org.rakishou.cachingimageloader.cache info file (${lines.size})")
      hasCorruptedEntries = true
    }
